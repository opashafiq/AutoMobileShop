...
export default function TireSaleReport() {
  ...
  const [query, setQuery] = useState<{
    ...
    brand: string
    size: string
  } | null>(() => ({ from: defaults.from, to: defaults.to, brand: '', size: '' }))

  /* ── Build API URL from the applied query (null key = no fetch) ── */
  const apiUrl = useMemo(() => {
    if (!query) return null
    const base = getApiUrl('/api/Reports/GetTireSaleReport')
  ...
  /* ── Fetch data ── */
  const { data: tireData, isLoading } = useSWR<TireSaleRow[]>(
    apiUrl,
    getFetcher,
    { refreshInterval: 0 }
  )
  ...
  const reportData = useMemo(
    () => (Array.isArray(tireData) ? tireData : []),
    [tireData]
  )
  ...
  /* ── Column definitions ── */
  const columnHelper = createColumnHelper<TireSaleRow>()
  ...
  /* ── Column filter keys ── */
  const filterableColumns = ['category', 'brand', 'size']

  /* ── Search: apply the draft inputs to the query and fetch ── */
  const handleSearch = () => {
    setQuery({ from: inputFrom, to: inputTo, brand: inputBrand, size: inputSize })
  }
  ...
  const handleExportExcel = useCallback(() => {
    const headers = ['Category', 'Brand', 'Size', 'Qty']
    ...
    link.href = url
    link.setAttribute('download', 'Tire_Sale_Report.csv')
      ...
      <div className='no-print-same' style={{ position: 'absolute', left: '-99999px', top: 0 }}>
        <div
          ...
          <div className='text-center mb-3'>
            <h2 className='text-lg font-bold text-slate-900 uppercase tracking-wide'>Tire Sale Report</h2>
            {(query?.from || query?.to) && (
