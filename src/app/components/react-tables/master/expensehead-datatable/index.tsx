...
import type { ExpenseHeadType } from '@/app/models/interfaces'
import {
...
function ExpenseHeadTable({ enableColumnFilters = true }: { enableColumnFilters?: boolean }) {
  const { activeMode } = useContext(CustomizerContext)

  const [expenseHeadData, setExpenseHeadData] = useState<ExpenseHeadType[]>([])

  // SWR fetch from API
  const API_URL = getApiUrl('/api/ExpenseHead')
  const { data } = useSWR(API_URL, getFetcher)

  useEffect(() => {
    if (data && Array.isArray(data)) {
      setExpenseHeadData(data as ExpenseHeadType[])
    }
  }, [data])
  ...
  const [currentExpenseHead, setCurrentExpenseHead] = useState<Partial<ExpenseHeadType>>({
    id: 0,
    tbeh_HeadName: '',
  })

  const resetCurrentExpenseHead = () => {
    setCurrentExpenseHead({
      id: 0,
      tbeh_HeadName: '',
    })
  }
  ...
  const openCreateDialog = () => {
    resetCurrentExpenseHead()
    setDialogMode('create')
    setEditingRowId(null)
    setIsDialogOpen(true)
  }

  const openEditDialog = (row: ExpenseHeadType) => {
    setDialogMode('edit')
    setEditingRowId(row.id)
    setCurrentExpenseHead({ ...row })
    setIsDialogOpen(true)
  }
  ...
  const handleSave = async () => {
    ...
    try {
      const payload: ExpenseHeadType = {
        id: dialogMode === 'create' ? 0 : editingRowId ?? 0,
        tbeh_HeadName: currentExpenseHead.tbeh_HeadName ?? '',
      ...
      if (dialogMode === 'create') {
        ...
        setExpenseHeadData((prev) => [response as ExpenseHeadType, ...prev])
        setFeedback('Expense Head created')
      } else if (editingRowId !== null) {
        const updatedExpenseHead = (await putFetcher(`${API_URL}/${editingRowId}`, payload)) as ExpenseHeadType | null
        setExpenseHeadData((prev) =>
          prev.map((item) =>
            item.id === editingRowId ? (updatedExpenseHead ?? payload) : item
          )
        )
        setFeedback('Expense Head updated')
      }

      closeDialog()
    } catch (error) {
      console.error('Unable to save expense head', error)
      setFeedback('Unable to save Expense Head')
    } finally {
      setApiLoading(false)
    }
  }
  ...
  const handleConfirmDelete = async () => {
    if (confirmDeleteCount > 1) {
      ...
      setExpenseHeadData((prev) => prev.filter((item) => !selectedIds.includes(item.id)))
    ...
    try {
      ...
      setExpenseHeadData((prev) => prev.filter((item) => item.id !== confirmDeleteTargetId))
  ...
  // Create column helper for ExpenseHeadType
  const columnHelper = createColumnHelper<ExpenseHeadType>()

  // Build columns matching the ExpenseHead API
  const allColumns = useMemo(
    () =>
      [
        ...
        columnHelper.accessor('tbeh_HeadName', {
          header: 'Head Name',
  ...
  const filteredData = useMemo(
    () =>
      applyColumnFilters(
        expenseHeadData as unknown as Record<string, unknown>[],
        columnFilters
      ) as unknown as ExpenseHeadType[],
    [expenseHeadData, columnFilters]
  )
  ...
  const visibleExportKeys = useMemo(
    () =>
      visibleColumns
        ...
        .map((col) => (col as any).accessorKey as keyof ExpenseHeadType),
    [visibleColumns]
  )
  ...
  const handleExportCSV = useCallback(() => {
    ...
    link.href = url
    link.setAttribute('download', 'expense-heads.csv')
      ...
      <div>
          ...
          <div className='flex flex-wrap items-center gap-1 md:gap-2'>
            ...
            {!showSearch ? (
              ...
              <Input
                ...
                aria-label='Search expense heads'
              />
            )}
            ...
            <Button
              ...
              aria-label='Create Expense Head'>
              Create Expense Head
            </Button>
          </div>
        </div>
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>
                {dialogMode === 'create' ? 'Create Expense Head' : 'Edit Expense Head'}
              </DialogTitle>
              <DialogDescription>
                {dialogMode === 'create'
                  ? 'Enter new expense head information and save to the database.'
                  : 'Update the expense head record and save your changes.'}
              </DialogDescription>
            </DialogHeader>
            ...
            <div className='grid gap-4 py-4'>
              <div className='grid gap-2'>
                ...
                <Input
                  value={currentExpenseHead.tbeh_HeadName ?? ''}
                  onChange={(e) =>
                    setCurrentExpenseHead((prev) => ({
                      ...prev,
                      tbeh_HeadName: e.target.value,
                    }))
                  }
            ...
            <DialogFooter>
              ...
              <Button
                ...
                variant='success'
                onClick={handleSave}
                disabled={apiLoading || !currentExpenseHead.tbeh_HeadName}>
                {apiLoading
                  ? dialogMode === 'create'
                    ? 'Creating...'
                    : 'Saving...'
                      ...
                      {headerGroup.headers.map((header) => {
                        ...
                        const columnData =
                          columnKey && enableColumnFilters
                            ? expenseHeadData.map((row) => row[columnKey as keyof ExpenseHeadType])
                            : []
                        const isFilterable =
                          enableColumnFilters &&
                          columnKey &&
                          columnKey !== 'id' &&
                          ['tbeh_HeadName'].includes(
                            columnKey
                          )
