...
import type { LocationDetailsType, CompanyInfoType } from '@/app/models/interfaces'
import {
...
function LocationDetailsTable({ enableColumnFilters = true }: { enableColumnFilters?: boolean }) {
  const { activeMode } = useContext(CustomizerContext)

  const [locationData, setLocationData] = useState<LocationDetailsType[]>([])

  const API_URL = getApiUrl('/api/LocationDetails')
  const { data } = useSWR(API_URL, getFetcher)

  // Fetch CompanyInfo for dropdown
  const COMPANY_API_URL = getApiUrl('/api/CompanyInfo')
  const { data: companyData } = useSWR(COMPANY_API_URL, getFetcher)

  useEffect(() => {
    if (data && Array.isArray(data)) {
      setLocationData(data as LocationDetailsType[])
    }
  }, [data])
  ...
  const [currentLocation, setCurrentLocation] = useState<Partial<LocationDetailsType>>({
    id: 0,
    tbld_LocationName: '',
    companyInfoId: 0,
    tbld_Address1: '',
  ...
  const companyOptions = useMemo(() => {
    ...
    return companyData as CompanyInfoType[]
  }, [companyData])
  ...
  const openEditDialog = (row: LocationDetailsType) => {
    setDialogMode('edit')
  ...
  const handleCompanyChange = (value: string) => {
    ...
    const selected = companyOptions.find((opt) => opt.id === id)
    setCurrentLocation((prev) => ({
      ...prev,
      companyInfoId: id,
      companyName: selected?.tbbiBusinessName ?? '',
    }))
  }
  ...
  const handleSave = async () => {
    setApiLoading(true)
    try {
      const payload: LocationDetailsType = {
        id: dialogMode === 'create' ? 0 : editingRowId ?? 0,
        tbld_LocationName: currentLocation.tbld_LocationName ?? '',
        companyInfoId: currentLocation.companyInfoId ?? 0,
        tbld_Address1: currentLocation.tbld_Address1 ?? '',
        tbld_Address2: currentLocation.tbld_Address2 ?? '',
        tbld_City: currentLocation.tbld_City ?? '',
        tbld_State: currentLocation.tbld_State ?? '',
        tbld_ZipCode: currentLocation.tbld_ZipCode ?? '',
        tbld_Phone: currentLocation.tbld_Phone ?? '',
        tbld_Fax: currentLocation.tbld_Fax ?? '',
        tbld_Email: currentLocation.tbld_Email ?? '',
      ...
      if (dialogMode === 'create') {
        ...
        setLocationData((prev) => [response as LocationDetailsType, ...prev])
        setFeedback('Location Details created')
      } else if (editingRowId !== null) {
        const updated = (await putFetcher(`${API_URL}/${editingRowId}`, payload)) as LocationDetailsType | null
        setLocationData((prev) =>
          prev.map((item) => (item.id === editingRowId ? (updated ?? payload) : item))
        )
        setFeedback('Location Details updated')
      }
      closeDialog()
    } catch (error) {
      console.error('Unable to save location details', error)
      setFeedback('Unable to save Location Details')
  ...
  const openDeleteConfirm = (rowId: number | string) => {
    ...
    setConfirmDeleteMessage('Are you sure you want to delete this Location? This action cannot be undone.')
  ...
  const handleConfirmDelete = async () => {
    if (confirmDeleteCount > 1) {
      ...
      setLocationData((prev) => prev.filter((item) => !selectedIds.includes(item.id)))
    ...
    try {
      ...
      setLocationData((prev) => prev.filter((item) => item.id !== confirmDeleteTargetId))
  ...
  const columnHelper = createColumnHelper<LocationDetailsType>()
    ...
    () =>
      [
        columnHelper.display({
          id: 'select',
          header: ({ table }) => (
            <Checkbox checked={table.getIsAllPageRowsSelected()} onCheckedChange={(checked) => table.toggleAllPageRowsSelected(checked === true)} />
          ),
          cell: ({ row }) => (
            <Checkbox checked={!!row.getIsSelected()} onCheckedChange={(checked) => row.toggleSelected(checked === true)} />
          ),
        }),
        columnHelper.accessor('tbld_LocationName', {
        ...
        columnHelper.accessor('tbld_Address1', {
          header: 'Address',
          cell: (info) => <p className='text-sm'>{info.getValue() || '-'}</p>,
        }),
        columnHelper.accessor('tbld_Phone', {
          header: 'Phone',
          cell: (info) => <p className='text-sm'>{info.getValue() || '-'}</p>,
        }),
        columnHelper.accessor('tbld_Email', {
          header: 'Email',
          cell: (info) => <p className='text-sm'>{info.getValue() || '-'}</p>,
        }),
        columnHelper.display({
            ...
            return (
              <DropdownMenu>
                ...
                <DropdownMenuContent className='shadow dark:shadow-white/20'>
                  <DropdownMenuItem onClick={() => openEditDialog(row.original)} className='cursor-pointer'>
                    <Icon icon='solar:pen-2-linear' width={20} height={20} className='me-2' /> Edit
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => handleDelete(rowId)} className="cursor-pointer text-red-600 focus:text-red-700">
                    <Icon icon='solar:trash-bin-2-outline' width={20} height={20} className='me-2' /> Delete
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
            )
      ...
      ] as ColumnDef<LocationDetailsType>[],
    []
  )
  ...
  const filteredData = useMemo(
    () => applyColumnFilters(locationData as unknown as Record<string, unknown>[], columnFilters) as unknown as LocationDetailsType[],
    [locationData, columnFilters]
  )
  ...
  const visibleExportKeys = useMemo(
    () => visibleColumns.filter((col) => (col as any).accessorKey && col.id !== 'select' && col.id !== 'actions').map((col) => (col as any).accessorKey as keyof LocationDetailsType),
    [visibleColumns]
  )
  ...
  const handleExportCSV = useCallback(() => {
    ...
    link.href = url; link.setAttribute('download', 'location-details.csv')
    ...
    <Card>
      <div>
        <div className='flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between mb-5'>
          <h3 className='text-lg font-semibold text-dark dark:text-white mb-4 md:mb-0'>Manage Location Details</h3>
          <div className='flex flex-wrap items-center gap-1 md:gap-2'>
            {!showSearch ? (
              ...
              <Input placeholder='Search...' className='form-control! w-40 md:w-56' value={globalFilter ?? ''} onChange={(e) => setGlobalFilter(e.target.value)} onBlur={() => { if (!globalFilter) setShowSearch(false) }} aria-label='Search locations' />
            )}
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
            ...
            <Button variant='lightprimary' shape='pill' onClick={openCreateDialog} aria-label='Create Location'>Create Location</Button>
          </div>
        </div>
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogContent className='sm:max-w-lg'>
            <DialogHeader>
              ...
              <DialogDescription>{dialogMode === 'create' ? 'Enter new location details and save to the database.' : 'Update the location record and save your changes.'}</DialogDescription>
            </DialogHeader>
            <div className='grid gap-4 py-4'>
              <div className='grid gap-2'>
                <label className='text-sm font-medium'>Location Name</label>
                <Input value={currentLocation.tbld_LocationName ?? ''} onChange={(e) => setCurrentLocation((p) => ({ ...p, tbld_LocationName: e.target.value }))} placeholder='Location Name' />
              </div>
              <div className='grid gap-2'>
                <label className='text-sm font-medium'>Company</label>
                <Select value={currentLocation.companyInfoId ? String(currentLocation.companyInfoId) : ''} onValueChange={handleCompanyChange}>
                  <SelectTrigger><SelectValue placeholder='Select company' /></SelectTrigger>
                  <SelectContent>
                    {companyOptions.map((opt) => (
                      <SelectItem key={opt.id} value={String(opt.id)}>{opt.tbbiBusinessName}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
              <div className='grid grid-cols-2 gap-4'>
                <div className='grid gap-2'>
                  ...
                  <Input value={currentLocation.tbld_Address1 ?? ''} onChange={(e) => setCurrentLocation((p) => ({ ...p, tbld_Address1: e.target.value }))} placeholder='Address 1' />
                </div>
                <div className='grid gap-2'>
                  <label className='text-sm font-medium'>Address 2</label>
                  <Input value={currentLocation.tbld_Address2 ?? ''} onChange={(e) => setCurrentLocation((p) => ({ ...p, tbld_Address2: e.target.value }))} placeholder='Address 2' />
                </div>
              </div>
              <div className='grid grid-cols-2 gap-4'>
                <div className='grid gap-2'>
                  <label className='text-sm font-medium'>City</label>
                  <Input value={currentLocation.tbld_City ?? ''} onChange={(e) => setCurrentLocation((p) => ({ ...p, tbld_City: e.target.value }))} placeholder='City' />
                </div>
                <div className='grid gap-2'>
                  <label className='text-sm font-medium'>State</label>
                  <Input value={currentLocation.tbld_State ?? ''} onChange={(e) => setCurrentLocation((p) => ({ ...p, tbld_State: e.target.value }))} placeholder='State' />
                </div>
              </div>
              <div className='grid grid-cols-2 gap-4'>
                <div className='grid gap-2'>
                  <label className='text-sm font-medium'>Zip Code</label>
                  <Input value={currentLocation.tbld_ZipCode ?? ''} onChange={(e) => setCurrentLocation((p) => ({ ...p, tbld_ZipCode: e.target.value }))} placeholder='Zip Code' />
                </div>
                <div className='grid gap-2'>
                  <label className='text-sm font-medium'>Phone</label>
                  <Input value={currentLocation.tbld_Phone ?? ''} onChange={(e) => setCurrentLocation((p) => ({ ...p, tbld_Phone: e.target.value }))} placeholder='Phone' />
                </div>
              </div>
              <div className='grid grid-cols-2 gap-4'>
                <div className='grid gap-2'>
                  <label className='text-sm font-medium'>Fax</label>
                  <Input value={currentLocation.tbld_Fax ?? ''} onChange={(e) => setCurrentLocation((p) => ({ ...p, tbld_Fax: e.target.value }))} placeholder='Fax' />
                </div>
                <div className='grid gap-2'>
                  <label className='text-sm font-medium'>Email</label>
                  <Input type='email' value={currentLocation.tbld_Email ?? ''} onChange={(e) => setCurrentLocation((p) => ({ ...p, tbld_Email: e.target.value }))} placeholder='Email' />
                </div>
              </div>
            </div>
            <DialogFooter>
              <Button type='button' variant='secondary' onClick={closeDialog} disabled={apiLoading}>Cancel</Button>
              <Button type='button' variant='success' onClick={handleSave} disabled={apiLoading || !currentLocation.tbld_LocationName}>
              ...
              <table className='min-w-full w-full'>
                  ...
                  {table.getHeaderGroups().map((headerGroup) => (
                    <tr key={headerGroup.id}>
                      {headerGroup.headers.map((header) => {
                        ...
                        const columnData = columnKey && enableColumnFilters ? locationData.map((row) => row[columnKey as keyof LocationDetailsType]) : []
                        const isFilterable = enableColumnFilters && columnKey && columnKey !== 'id' && ['tbld_LocationName', 'companyName', 'tbld_Address1', 'tbld_Phone', 'tbld_Email'].includes(columnKey)
                        return (
                ...
                <AnimatedTableBody>
                  {table.getRowModel().rows.length === 0 ? (
                    <tr><td colSpan={visibleColumns.length} className='text-center py-4'>No locations found.</td></tr>
                  ) : (
                      ...
                      <React.Fragment key={row.id}>
                        <AnimatedTableRow index={index} className='border-b last:border-b-0 border-ld hover:bg-lightprimary transition-colors duration-200'>
                          {row.getVisibleCells().map((cell) => (
                            <td key={cell.id} className={`px-4 py-2 ${cell.column.id === 'tbld_LocationName' || cell.column.id === 'companyName' ? 'min-w-[180px]' : ''}`}>
