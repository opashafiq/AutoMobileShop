...
import type { RoleType } from '@/app/models/interfaces'
import {
...
function RolesTable({ enableColumnFilters = true }: { enableColumnFilters?: boolean }) {
  const { activeMode } = useContext(CustomizerContext)

  const [roleData, setRoleData] = useState<RoleType[]>([])

  // Roles API is under /Authentication/roles (not /api)
  const API_URL = getApiUrl('/Authentication/roles')
  ...
  const [currentRole, setCurrentRole] = useState<{ roleName: string; id?: string }>({
    roleName: '',
    id: '',
  })

  const resetCurrentRole = () => {
    setCurrentRole({ roleName: '', id: '' })
  }
  ...
  const openEditDialog = (row: RoleType) => {
    setDialogMode('edit')
    setEditingRowId(row.id)
    setCurrentRole({ roleName: row.name, id: row.id })
    setIsDialogOpen(true)
  }
  ...
  const handleSave = async () => {
    ...
    try {
      // Roles API uses { roleName: "..." } as payload (not full RoleType)
      const payload = { roleName: currentRole.roleName ?? '' }

      if (dialogMode === 'create') {
        const response = await postFetcher(API_URL, payload)
        setRoleData((prev) => [response as RoleType, ...prev])
        setFeedback('Role created')
      } else if (editingRowId !== null) {
        const updatedRole = (await putFetcher(`${API_URL}/${editingRowId}`, payload)) as RoleType | null
        setRoleData((prev) =>
          prev.map((item) =>
            item.id === editingRowId ? (updatedRole ?? { ...item, name: payload.roleName }) : item
          )
        )
      ...
      setFeedback('Unable to save Role')
    } finally {
      setApiLoading(false)
    }
  }
  ...
  const handleConfirmDelete = async () => {
    if (confirmDeleteCount > 1) {
      ...
      setRoleData((prev) => prev.filter((item) => !selectedIds.includes(item.id)))
    ...
    try {
      ...
      setRoleData((prev) => prev.filter((item) => item.id !== confirmDeleteTargetId))
  ...
  // Create column helper for RoleType
  const columnHelper = createColumnHelper<RoleType>()

  // Build columns matching the Roles API
  ...
  const visibleExportKeys = useMemo(
    () =>
      visibleColumns
        ...
        .map((col) => (col as any).accessorKey as keyof RoleType),
    [visibleColumns]
  )
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                {dialogMode === 'create'
                  ? 'Enter the role name and save to the database.'
            ...
            <DialogFooter>
              ...
              <Button
                ...
                variant='success'
                onClick={handleSave}
                disabled={apiLoading || !currentRole.roleName}>
                {apiLoading
                  ? dialogMode === 'create'
                    ? 'Creating...'
                    : 'Saving...'
                      ...
                      {headerGroup.headers.map((header) => {
                        ...
                        const columnData =
                          columnKey && enableColumnFilters
                            ? roleData.map((row) => row[columnKey as keyof RoleType])
                            : []
                        const isFilterable =
                          enableColumnFilters &&
                          columnKey &&
