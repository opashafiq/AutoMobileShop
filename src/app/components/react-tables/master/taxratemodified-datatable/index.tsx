...
import type { TaxRateModifiedType } from '@/app/models/interfaces'
import {
...
function TaxRateModifiedTable({ enableColumnFilters = true }: { enableColumnFilters?: boolean }) {
  const { activeMode } = useContext(CustomizerContext)

  const [taxRateData, setTaxRateData] = useState<TaxRateModifiedType[]>([])

  const API_URL = getApiUrl('/api/TaxRateModified')
  ...
  const openEditDialog = (row: TaxRateModifiedType) => { setDialogMode('edit'); setEditingRowId(row.id); setCurrentTaxRate({ ...row }); setIsDialogOpen(true) }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error('Unable to save tax rate', error)
      setFeedback('Unable to save Tax Rate')
    } finally { setApiLoading(false) }
  }

  const openDeleteConfirm = (rowId: number | string) => {
    setConfirmDeleteTargetId(rowId); setConfirmDeleteCount(1)
    setConfirmDeleteMessage('Are you sure you want to delete this Tax Rate? This action cannot be undone.')
  ...
  const handleConfirmDelete = async () => {
    if (confirmDeleteCount > 1) {
      ...
      setTaxRateData((prev) => prev.filter((item) => !selectedIds.includes(item.id)))
    ...
    try {
      ...
      setTaxRateData((prev) => prev.filter((item) => item.id !== confirmDeleteTargetId))
  ...
  const columnHelper = createColumnHelper<TaxRateModifiedType>()
  ...
  const allColumns = useMemo(
    () =>
      ...
      ] as ColumnDef<TaxRateModifiedType>[],
    []
  )
  ...
  const filteredData = useMemo(
    () => applyColumnFilters(taxRateData as unknown as Record<string, unknown>[], columnFilters) as unknown as TaxRateModifiedType[],
    [taxRateData, columnFilters]
  )
  ...
  const visibleExportKeys = useMemo(
    () => visibleColumns.filter((col) => (col as any).accessorKey && col.id !== 'select' && col.id !== 'actions').map((col) => (col as any).accessorKey as keyof TaxRateModifiedType),
    [visibleColumns]
  )
  ...
  const handleExportCSV = useCallback(() => {
    ...
    link.href = url; link.setAttribute('download', 'tax-rate-modified.csv')
    ...
    <Card>
      <div>
        <div className='flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between mb-5'>
          <h3 className='text-lg font-semibold text-dark dark:text-white mb-4 md:mb-0'>Manage Tax Rate Modified</h3>
          <div className='flex flex-wrap items-center gap-1 md:gap-2'>
            {!showSearch ? (
              ...
              <Input placeholder='Search...' className='form-control! w-40 md:w-56' value={globalFilter ?? ''} onChange={(e) => setGlobalFilter(e.target.value)} onBlur={() => { if (!globalFilter) setShowSearch(false) }} aria-label='Search tax rates' />
            )}
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
            ...
            <Button variant='lightprimary' shape='pill' onClick={openCreateDialog} aria-label='Create Tax Rate'>Create Tax Rate</Button>
          </div>
        </div>
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{dialogMode === 'create' ? 'Create Tax Rate' : 'Edit Tax Rate'}</DialogTitle>
              <DialogDescription>{dialogMode === 'create' ? 'Enter new tax rate information and save to the database.' : 'Update the tax rate record and save your changes.'}</DialogDescription>
            </DialogHeader>
            ...
            <DialogFooter>
              ...
              <Button type='button' variant='success' onClick={handleSave} disabled={apiLoading || !currentTaxRate.tbtm_Note}>
              ...
              <table className='min-w-full w-full'>
                ...
                <AnimatedTableBody>
                  {table.getRowModel().rows.length === 0 ? (
                    <tr><td colSpan={visibleColumns.length} className='text-center py-4'>No tax rates found.</td></tr>
                  ) : (
...
export default TaxRateModifiedTable
