...
import type { DepartmentType } from '@/app/models/interfaces'
import {
...
function DepartmentsTable({ enableColumnFilters = true }: { enableColumnFilters?: boolean }) {
  const { activeMode } = useContext(CustomizerContext)

  const [departmentData, setDepartmentData] = useState<DepartmentType[]>([])

  // SWR fetch from API
  const API_URL = getApiUrl('/api/Departments')
  const { data } = useSWR(API_URL, getFetcher)

  useEffect(() => {
    if (data && Array.isArray(data)) {
      setDepartmentData(data as DepartmentType[])
    }
  }, [data])
  ...
  const [currentDepartment, setCurrentDepartment] = useState<Partial<DepartmentType>>({
    id: 0,
    tbid_DepartmentName: '',
    tbid_IsActive: true,
  })
  ...
  const openEditDialog = (row: DepartmentType) => {
    setDialogMode('edit')
  ...
  const handleSave = async () => {
    ...
    try {
      const payload: DepartmentType = {
        ...
        tbid_DepartmentName: currentDepartment.tbid_DepartmentName ?? '',
        tbid_IsActive: currentDepartment.tbid_IsActive ?? true,
      ...
      if (dialogMode === 'create') {
        ...
        setDepartmentData((prev) => [response as DepartmentType, ...prev])
        setFeedback('Department created')
      } else if (editingRowId !== null) {
        const updatedDepartment = (await putFetcher(`${API_URL}/${editingRowId}`, payload)) as DepartmentType | null
        setDepartmentData((prev) =>
          prev.map((item) =>
            item.id === editingRowId ? (updatedDepartment ?? payload) : item
          )
        )
      ...
      console.error('Unable to save department', error)
      setFeedback('Unable to save Department')
    } finally {
      setApiLoading(false)
    }
  }
  ...
  const handleConfirmDelete = async () => {
    if (confirmDeleteCount > 1) {
      ...
      setDepartmentData((prev) => prev.filter((item) => !selectedIds.includes(item.id)))
    ...
    try {
      ...
      setDepartmentData((prev) => prev.filter((item) => item.id !== confirmDeleteTargetId))
  ...
  // Create column helper for DepartmentType
  const columnHelper = createColumnHelper<DepartmentType>()

  // Build columns matching the Departments API
  const allColumns = useMemo(
    () =>
      [
        ...
        columnHelper.accessor('tbid_DepartmentName', {
          header: 'Department Name',
          cell: (info) => <p className='text-sm font-medium'>{info.getValue()}</p>,
        }),

        columnHelper.accessor('tbid_IsActive', {
          header: 'Active',
          cell: (info) => {
  ...
  const visibleExportKeys = useMemo(
    () =>
      visibleColumns
        ...
        .map((col) => (col as any).accessorKey as keyof DepartmentType),
    [visibleColumns]
  )
  ...
  const handleExportCSV = useCallback(() => {
    const rows = filteredData.map((row) =>
      visibleExportKeys.map((key) => {
        const val = row[key]
        if (key === 'tbid_IsActive') return val ? 'Active' : 'Inactive'
        return val ?? ''
      })
    )
    const csvContent = [
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                {dialogMode === 'create'
                  ? 'Enter new department information and save to the database.'
                  : 'Update the department record and save your changes.'}
              </DialogDescription>
            </DialogHeader>
            ...
            <div className='grid gap-4 py-4'>
              <div className='grid gap-2'>
                ...
                <Input
                  value={currentDepartment.tbid_DepartmentName ?? ''}
                  onChange={(e) =>
                    setCurrentDepartment((prev) => ({
                      ...prev,
                      tbid_DepartmentName: e.target.value,
                    }))
                  }
              ...
              <div className='flex items-center justify-between rounded-lg border p-3'>
                <div>
                  ...
                  <p className='text-xs text-muted-foreground'>
                    {currentDepartment.tbid_IsActive ? 'Department is active' : 'Department is inactive'}
                  </p>
                </div>
                <Switch
                  checked={currentDepartment.tbid_IsActive ?? true}
                  onCheckedChange={(checked) =>
                    setCurrentDepartment((prev) => ({
                      ...prev,
                      tbid_IsActive: checked,
                    }))
                  }
                />
              </div>
            ...
            <DialogFooter>
              ...
              <Button
                ...
                variant='success'
                onClick={handleSave}
                disabled={apiLoading || !currentDepartment.tbid_DepartmentName}>
                {apiLoading
                  ? dialogMode === 'create'
                    ? 'Creating...'
                    : 'Saving...'
                      ...
                      {headerGroup.headers.map((header) => {
                        ...
                        const columnData =
                          columnKey && enableColumnFilters
                            ? departmentData.map((row) => row[columnKey as keyof DepartmentType])
                            : []
                        const isFilterable =
                          enableColumnFilters &&
                          columnKey &&
                          columnKey !== 'id' &&
                          ['tbid_DepartmentName', 'tbid_IsActive'].includes(
                            columnKey
                          )
                        ...
                        <AnimatedTableRow
                          ...
                          {row.getVisibleCells().map((cell) => (
                            <td
                              key={cell.id}
                              className={`px-4 py-2 ${
                                cell.column.id === 'tbid_DepartmentName'
                                  ? 'min-w-[180px]'
                                  : ''
                              }`}>
                              {flexRender(
