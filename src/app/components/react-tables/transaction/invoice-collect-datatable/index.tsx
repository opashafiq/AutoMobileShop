...
export default function InvoiceCollectDatatable() {
        ...
        cell: ({ row }) => {
          ...
          return (
            <Button
              size='sm'
              variant='default'
              onClick={() => router.push(`/react-tables/transaction/invoice-collect/${invId}`)}
            >
  ...
  const handleExportCSV = useCallback(() => {
    ...
    a.href = url; a.download = 'pending-collection.csv'
      ...
      <div>
        ...
        {error && (
          <div className='mb-4 rounded-lg border border-error/30 bg-error/5 p-4 text-sm text-error'>
            Failed to load pending invoices. Please ensure the backend API is reachable.
          </div>
        )}
                  ...
                  {table.getRowModel().rows.length === 0 ? (
                    <tr>
                      <td colSpan={visibleColumns.length} className='text-center py-8 text-sm text-darklink dark:text-bodytext'>
                        {isLoading ? 'Loading pending invoices...' : 'No pending invoices found.'}
                      </td>
                    </tr>
                  ) : (
