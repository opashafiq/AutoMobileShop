...
import {
  type LayawayRefundListResponse,
...
// Map a column id to the nested path on LayawayRefundListResponseItem for filter data extraction.
const MASTER_ACCESSORS: Record<string, string> = {
  transactionId: 'tbirm_LayawayRefundIdRad',
  customerName: 'originalLayawayName',
  date: 'tbirm_LayawayRefundDate',
...
const columnHelper = createColumnHelper<LayawayRefundListResponseItem>()

export default function LayawayRefundDatatable() {
  ...
  const API_URL = getApiUrl(`/api/LayawayRefundMaster?${queryString}`)
  const { data, isLoading, error, mutate } = useSWR<LayawayRefundListResponse>(API_URL, getFetcher)

  const [tableData, setTableData] = useState<LayawayRefundListResponseItem[]>([])
  ...
  // Delete state
  const [deleteTarget, setDeleteTarget] = useState<LayawayRefundListResponseItem | null>(null)
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true)
    try {
      const id = deleteTarget.layawayRefundMasterDto.id
      await deleteFetcher(`${getApiUrl('/api/LayawayRefundMaster')}/${id}`)
      if (tableData.length === 1 && pageNumber > 1) setPageNumber((p) => p - 1)
      else mutate()
      setFeedback('Layaway refund deleted')
      setDeleteTarget(null)
    } catch {
      setFeedback('Failed to delete layaway refund')
    } finally {
      setIsDeleting(false)
    }
  }
  ...
  // ---- columns ----
  const allColumns = useMemo<ColumnDef<LayawayRefundListResponseItem>[]>(() => {
    return [
      ...
      columnHelper.display({
        ...
        cell: ({ row }) => (
          <span className='font-semibold text-primary'>{row.original.layawayRefundMasterDto.tbirm_LayawayRefundIdRad || '-'}</span>
        ),
      }),
      columnHelper.display({
        id: 'customerName',
        header: 'Customer Name',
        cell: ({ row }) => <p className='text-sm'>{row.original.layawayRefundMasterDto.originalLayawayName || '-'}</p>,
      }),
      columnHelper.display({
        id: 'date',
        header: 'Date',
        cell: ({ row }) => <p className='text-sm'>{formatDate(row.original.layawayRefundMasterDto.tbirm_LayawayRefundDate)}</p>,
      }),
      columnHelper.display({
        id: 'totalAmount',
        header: 'Total Amount',
        cell: ({ row }) => <p className='text-sm font-semibold'>${formatMoney(row.original.layawayRefundMasterDto.tbirm_Total)}</p>,
      }),
      columnHelper.display({
        id: 'phone',
        header: 'Phone No',
        cell: ({ row }) => <p className='text-sm'>{row.original.layawayRefundMasterDto.tbim_Phone || '-'}</p>,
      }),
      columnHelper.display({
        id: 'refundAmount',
        header: 'Refund Amount',
        cell: ({ row }) => <p className='text-sm'>${formatMoney(row.original.layawayRefundMasterDto.tbirm_RefundAmt)}</p>,
      }),
      columnHelper.display({
        id: 'actions',
        enableSorting: false,
        header: 'Actions',
        cell: ({ row }) => {
          const refundId = row.original.layawayRefundMasterDto.id
          return (
            <DropdownMenu>
              ...
              <DropdownMenuContent className='shadow dark:shadow-white/20'>
                <DropdownMenuCheckboxItem
                  onClick={() => router.push(`/react-tables/transaction/layaway-refund/${refundId}/edit`)}
    ...
    ] as ColumnDef<LayawayRefundListResponseItem>[]
  }, [router])
    ...
    for (const colId of FILTERABLE_COLUMNS) {
      ...
      if (accessor) {
        map[colId] = tableData.map((row) => {
          const v = (row.layawayRefundMasterDto as any)[accessor]
  ...
  const filteredData = useMemo(
    () => applyColumnFilters(
      ...
      columnFilters,
    ) as unknown as LayawayRefundListResponseItem[],
  ...
  const handleBulkDelete = useCallback(() => {
    const selectedIds = table.getSelectedRowModel().rows.map((r) => r.original.layawayRefundMasterDto.id)
    if (selectedIds.length === 0) return
    selectedIds.forEach((id) => {
      deleteFetcher(`${getApiUrl('/api/LayawayRefundMaster')}/${id}`).catch(() => {})
    })
    if (selectedIds.length >= tableData.length && pageNumber > 1) setPageNumber((p) => p - 1)
    else mutate()
    table.resetRowSelection()
    setFeedback(`Deleted ${selectedIds.length} layaway refund(s)`)
  ...
  const handleExportCSV = useCallback(() => {
    const rows = filteredData.map((row) =>
      visibleExportKeys.map((key) => {
        const m = row.layawayRefundMasterDto
        switch (key) {
          case 'transactionId': return m.tbirm_LayawayRefundIdRad
          case 'customerName': return m.originalLayawayName
          case 'date': return formatDate(m.tbirm_LayawayRefundDate)
    ...
    a.href = url; a.download = 'layaway-refunds.csv'
    ...
    <Card>
      <div>
          ...
          <div className='flex flex-wrap items-center gap-1 md:gap-2'>
            {!showSearch ? (
              ...
              <Input
                ...
                aria-label='Search layaway refunds'
              />
            )}
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
            ...
            <Button variant='lightprimary' shape='pill' onClick={() => router.push('/react-tables/transaction/layaway-refund/create')} aria-label='New Refund'>
              New Refund
            </Button>
          </div>
        </div>
        ...
        {error && (
          <div className='mb-4 rounded-lg border border-error/30 bg-error/5 p-4 text-sm text-error'>
            Failed to load layaway refunds. Please ensure the backend API is reachable.
          </div>
        )}
                  ...
                  {table.getRowModel().rows.length === 0 ? (
                    <tr>
                      <td colSpan={visibleColumns.length} className='text-center py-8 text-sm text-darklink dark:text-bodytext'>
                        {isLoading ? 'Loading layaway refunds...' : 'No layaway refunds found.'}
                      </td>
                    </tr>
                  ) : (
      ...
      <Dialog open={!!deleteTarget} onOpenChange={(open) => !open && setDeleteTarget(null)}>
        <DialogContent className='max-w-sm'>
          <DialogHeader>
            <DialogTitle>Delete Layaway Refund</DialogTitle>
            <DialogDescription>
              Are you sure you want to delete layaway refund{' '}
              <span className='font-semibold text-primary'>{deleteTarget?.layawayRefundMasterDto.tbirm_LayawayRefundIdRad}</span>
...
function FragmentRow({ row, index }: { row: any; index: number }) {
  const details: LayawayRefundDetailsDto[] = row.original.layawayRefundDetailsDto || []
  const master: LayawayRefundMasterDto = row.original.layawayRefundMasterDto

  const buildDescription = (d: LayawayRefundDetailsDto): string =>
