...
interface InvoiceRefundPickerProps {
  onSelect: (invoice: InvoiceMasterDto, mode: 'item' | 'payment') => void
  onCancel: () => void
}

export default function InvoiceRefundPicker({ onSelect, onCancel }: InvoiceRefundPickerProps) {
  ...
  const columns = useMemo<ColumnDef<InvoiceListResponseItem>[]>(() => [
    ...
    columnHelper.display({
      ...
      cell: ({ row }) => {
        const refunded = row.original.invoiceMasterDto.refundAmount || 0
    ...
    columnHelper.display({
      ...
      header: 'Refund Type',
      cell: ({ row }) => (
        <p className='text-sm'>{REFUND_TYPE_LABEL[row.original.invoiceMasterDto.tbim_RefundType] || '-'}</p>
      ),
    }),
    columnHelper.display({
      id: 'actions',
      header: 'Actions',
      cell: ({ row }) => {
        const m = row.original.invoiceMasterDto
        ...
        return (
          <div className='flex gap-1.5'>
            <Button
              ...
              title={isFullyRefunded ? 'Invoice is fully refunded' : `Item Refund (remaining: $${formatMoney(remaining)})`}
            >
            ...
            <Button
              ...
              title={isFullyRefunded ? 'Invoice is fully refunded' : `Payment Refund (remaining: $${formatMoney(remaining)})`}
            >
  ...
  ] as ColumnDef<InvoiceListResponseItem>[], [onSelect])
    ...
    <div className='space-y-5'>
      ...
      <Card className='p-4 md:p-5'>
        <div className='mb-4 flex items-center justify-between'>
          <div>
            <h4 className='text-lg font-semibold text-ld dark:text-darklink'>Select Invoice to Refund</h4>
            <p className='text-sm text-darklink dark:text-bodytext'>Choose an invoice and select whether to refund items, payments, or both.</p>
          </div>
        ...
        <div className='mb-4 rounded-lg border border-ld bg-lightprimary/10 p-4 dark:bg-darkinfo/5'>
          <div className='flex flex-wrap items-end gap-4'>
            <div className='min-w-[140px] flex-1'>
              ...
              <Input placeholder='Filter by invoice' value={invoiceTransactionId} onChange={(e) => setInvoiceTransactionId(e.target.value)} className='h-10' />
            </div>
            <div className='min-w-[140px] flex-1'>
