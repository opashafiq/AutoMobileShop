...
// ---------- Empty payment (layaway version — simpler DTO) ----------
function emptyPayment(layawayId: number): LayawayPaymentsDto {
  return {
    id: 0,
    tbip_InvoiceId: layawayId,
    tbip_PaymentId: 0,
    tbip_PayAmt: 0,
...
interface LayawayCollectFormProps {
  layawayId: string
}

export default function LayawayCollectForm({ layawayId }: LayawayCollectFormProps) {
  const router = useRouter()
  const laId = Number(layawayId)

  // ---- Fetch layaway data ----
  const { data: layawayData, isLoading } = useSWR<LayawayListResponseItem>(
  ...
  // ---- Local state ----
  const [newPayments, setNewPayments] = useState<LayawayPaymentsDto[]>([])
  const [sheetOpen, setSheetOpen] = useState(false)
  const [draftPayment, setDraftPayment] = useState<LayawayPaymentsDto>(emptyPayment(laId))
  const [saving, setSaving] = useState(false)
  const [hydrated, setHydrated] = useState(false)

  const master = layawayData?.layawayMasterDto

  // ---- Hydration ----
  useEffect(() => {
    if (layawayData && !hydrated) {
      setHydrated(true)
    }
  }, [layawayData, hydrated])
  ...
  const openAddPaymentSheet = () => {
    setDraftPayment({ ...emptyPayment(laId), tbip_Date: getLocalISO() })
    setSheetOpen(true)
  }
  ...
  const handleCollect = async () => {
    ...
    try {
      ...
      await postFetcher(getApiUrl('/api/LayawayMaster/SavePayments'), payload)
      toast.success('Payment collected successfully')
      router.push('/react-tables/transaction/layaway-collect')
    } catch (err) {
      toast.error(
  ...
  const details: LayawayDetailsDto[] = layawayData?.layawayDetailsDto ?? []
      ...
      <Card className='p-4 md:p-5'>
        <div>
          <h4 className='text-lg font-semibold text-ld dark:text-darklink'>
            Collect Payment — Layaway #{master.tbim_InvoiceIdRad}
          </h4>
          <p className='text-sm text-darklink dark:text-bodytext'>
            Record payments against this layaway to clear the pending amount.
          </p>
        </div>
      </Card>

      <div className='grid grid-cols-1 gap-5 xl:grid-cols-3'>
        {/* LEFT: read-only layaway summary */}
        ...
        <div className='space-y-5'>
          ...
          <Card className='p-4 md:p-5'>
            <div className='flex flex-col gap-2'>
              ...
              <Button variant='outline' onClick={() => router.push('/react-tables/transaction/layaway-collect')} disabled={saving} className='w-full'>
      ...
      <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
        <SheetContent side='right' className='w-full overflow-y-auto sm:max-w-md'>
          <SheetHeader className='px-4 pt-4'>
            ...
            <SheetDescription>
              Record a payment against this layaway. Maximum: ${money(master.pendingAmount)}.
            </SheetDescription>
          </SheetHeader>
