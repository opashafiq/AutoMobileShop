    ...
    <div className="w-full border-e border-ld">
      ...
      <ul className="my-4 mt-0 pt-4">
        {filterCategory.map(filter => filter.filterbyTitle ? (
          ...
          <li
            key={filter.id}
            className={`flex items-center gap-2 py-3 px-4 rounded-md cursor-pointer mx-6 ${selectedCategory === filter.sort
      ...
      {/* Sort */}
      <ul className="mt-0 px-6 pb-6">
        <h6 className="capitalize text-sm pb-4">Sort By</h6>
        {filterbySort.map(filter => (
          <li
            key={filter.id}
            className={`flex items-center gap-2 py-3 px-4 rounded-md cursor-pointer ${sortBy === filter.value
              ? "text-primary bg-lightprimary dark:bg-lightprimary"
              : "text-bodytext dark:text-darklink bg-hover"
              }`}
        ...
        >
          {genders.map(gender => (
            <div key={gender} className="flex items-center gap-3">
              ...
              <Label htmlFor={`gender-${gender}`} className="font-normal text-bodytext dark:text-darklink text-sm">
                {gender}
              </Label>
            </div>
          ))}
        ...
        >
          {prices.map(price => (
            <div key={price} className="flex items-center gap-3">
              ...
              <Label htmlFor={`price-${price}`} className="font-normal text-bodytext dark:text-darklink text-sm">
                {price}
              </Label>
            </div>
          ))}
